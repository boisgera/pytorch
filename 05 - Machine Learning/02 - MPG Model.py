...
def _():
    ...
    import ucimlrepo
    return pd, plt, requests, ucimlrepo
...
@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""### From the CSV file (with pandas)""")
    return
...
@app.cell
def _(df_1):
    list(df_1[df_1['origin'] == 1]['car_name'].values)
    return


@app.cell
def _(df_1):
    list(df_1[df_1['origin'] == 2]['car_name'].values)
    return


@app.cell
def _(df_1):
    list(df_1[df_1['origin'] == 3]['car_name'].values)
    return
...
def _(df_1):
    def region(i):
        if _i == 1:
            ...
            return 'Asia'
    df_1['origin'] = df_1['origin'].transform(region)
    df_1
    df_full = df_1
    return (df_full,)
...
@app.cell
def _(df_1, sns_1):
    sns_1.pairplot(df_1, height=2.0, hue='origin')
    None
    return
...
@app.cell
def _(df_1, torch):
    weight = torch.tensor(df_1['weight'].values).type(torch.float64)
    mpg = torch.tensor(df_1['mpg'].values)
