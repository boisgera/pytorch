...
def _(mo):
    ...
    `T_1 @ T_2` is equivalent to `tensordot(T_1, T_2, dim=1)` when `T_1` and `T_2` are 1d or 2d tensors.

    For higher ranks, extra dimensions are interpreted as batch dimensions and the behavior is therefore different; refer to [torch.matmul] for more details.
...
def _(mo):
    mo.md(r"""
    Consider (2d) batches of matrices of size $(4,5)$ and $(5,6)$:
    """)
    return
...
def _(mo):
    mo.md(r"""
    We can compute the batches of products with:
    """)
    return
...
def _(mo):
    mo.md(r"""
    The ellipsis `...` provide a convenient shortcut for the batched dimensions.
    """)
    return
